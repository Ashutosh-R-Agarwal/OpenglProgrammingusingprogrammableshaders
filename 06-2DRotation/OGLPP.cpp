...
void initialize(void)
{
	...
	glVertexAttrib3f(VDG_ATTRIBUTE_COLOR, 0.5f, 0.5f, 1.0f); 
...
void display(void)
{
	...
	myRotate(rotationMatrix, gAngleTriangle, 0.0f, 1.0f, 0.0f); 
	...
	myRotate(rotationMatrix, gAngleSquare, 1.0f, 0.0f, 0.0f);
...
void spin(void)
{
	...
	if (gAngleTriangle >= 360.0f)
		gAngleTriangle = gAngleTriangle - 360.0f;

	gAngleSquare = gAngleSquare + 1.0f;

	if (gAngleSquare >= 360.0f)
		gAngleSquare = gAngleSquare - 360.0f;
